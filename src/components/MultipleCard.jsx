...
const MultipleCard = ({ title, images, baseurl, topRated }) => {
  ...
  return (
    <Link
      href={`${baseurl}/${title}${topRated ? "?topRated=true" : ""}`}
      className="flex-shrink-0 3xl:w-72 md:w-60 w-44">
        ...
        <div className="mb-9">
          <div className="grid grid-cols-2 gap-1">
            ...
            {Array.from({ length: placeholders })?.map((_, index) => (
              <div
                key={index + displayImages.length}
                className="w-full bg-c-black-20 rounded-xl aspect-square"></div>
            ))}
          </div>
        </div>
        <div
