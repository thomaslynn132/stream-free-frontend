  ...
  const handleNext = () => {
    if (scrollContainerRef.current) {
      ...
      setCurrentIndex((prevIndex) =>
        Math.min(prevIndex + 1, series.length - 1)
      );
    }
  };
        ...
        <div className="flex items-center max-md:w-full max-md:justify-between">
          ...
          <span className="text-c-grey-90 ml-16 3xl:text-base xl:text-super-sm md:text-sm text-super-xs">
            <Link href={`/series/trending-now`}>
              See more{" "}
              <LeftArrowSvg className="inline stroke-c-grey-90 rotate-180 ml-1.5 md:w-[18px] w-4" />
            </Link>
          </span>
        </div>
        <SlidePagination
      ...
      <div
        ...
        {loading ? (
          ...
          <span className="3xl:text-super-base xl:text-super-sm max-md:text-sm text-c-grey-60">
            Sorry, no series available yet. Please visit us again later.
          </span>
        ) : (
          series?.map(
            (
              { _id, title, totalEpisodes, thumbnail, views, averageRating },
              index
            ) => (
              <MovieCard
                key={_id}
